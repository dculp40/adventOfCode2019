...
def _run(program, inputs, is_feedback=False, pc=0):
    ...
    while pc < len(program):
        ...
        if opcode == 1 or opcode == 2:
            op1, op2 = _get_op1(program, pc, op1_mode), _get_op2(
                program, pc, op2_mode)
            dest = program[pc + 3]
            ...
            op1, op2 = _get_op1(program, pc, op1_mode), _get_op2(
                program, pc, op2_mode)
            pc = op2 if (opcode == 5 and op1 != 0) or (
                opcode == 6 and op1 == 0) else pc + 3
        elif opcode == 7:
            op1, op2 = _get_op1(program, pc, op1_mode), _get_op2(
                program, pc, op2_mode)
...
def part_one():
    ...
    phase_perms = [[0, 0, 0, 0, 0]]  # _get_phase_permutations(range(0, 5))
