    ...
    for move in moves:
        ...
        if move[0] == 'R':
            for i in range(int(move[1:])):
                spaces.append((currX + i + 1, currY))
        elif move[0] == "U":
            for i in range(int(move[1:])):
                spaces.append((currX, currY + i + 1))
        elif move[0] == "D":
            for i in range(int(move[1:])):
                spaces.append((currX, currY - i - 1))
        else:  # "L"
...
# def crosses(listA, listB):
#     crosses = []
#     for item1 in listA[1:]:
#         for item2 in listB[1:]:
#             print("comparing " + str(item1) + " with " + str(item2))
#             if item1[0] == item2[0] & item1[1] == item2[1]:
#                 print(str(item1[0]) + " = " + str(item2[0]))
#                 crosses.append((item1[0], item1[1], item1[2] + item2[2]))
#                 print(crosses[-1])
#     return crosses


def leastManDistance(crosses):
    least = sys.maxint
...
def leastWireDistance(crosses, wire1, wire2):
    least = sys.maxint
    for cross in crosses:
        dist = wire1.index(cross) + wire2.index(cross)
        if dist != 0:
            least = min(least, dist)
    return least


spaces1 = traceWire(wire1)
# print(spaces1)
spaces2 = traceWire(wire2)
# print(spaces2)
crosses = set(spaces1).intersection(spaces2)
# print(crosses)


# crosses = crosses(spaces1, spaces2)
# print(crosses)

print(leastManDistance(crosses))
print(leastWireDistance(crosses, spaces1, spaces2))
